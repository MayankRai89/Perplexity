...
const Home = () => {
  ...
  const { isListening, startListening, stopListening, hasSupport } = useVoice();
  ...
  const handleSearchSubmit = (e) => {
    ...
    navigate(
      `/chat?q=${encodeURIComponent(finalQuery)}&focus=${encodeURIComponent(focusMode)}&pro=${isPro}`,
    );
  };
      ...
      <aside className="w-64 bg-[#191a1a] border-r border-[#2d3131]/30 flex flex-col p-4 justify-between hidden md:flex shrink-0">
        <div className="flex flex-col gap-6">
          ...
          >
            ...
            <span className="text-[10px] bg-[#2d3131] px-1.5 py-0.5 rounded text-slate-500 font-mono">
              Ctrl I
            </span>
          </button>
            ...
            <div className="flex flex-col gap-1.5 mt-2 border-t border-[#2d3131]/20 pt-4">
              ...
              <div className="flex flex-col gap-0.5 max-h-56 overflow-y-auto px-1.5 no-scrollbar">
                {threads.map((thread) => (
                  <div
                    key={thread._id}
                    className="group flex items-center justify-between hover:bg-[#202222]/60 rounded-lg py-1.5 px-2 transition"
                  >
                    <Link
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#131415] relative">
        ...
        <main className="flex-grow flex flex-col items-center justify-center px-4 md:px-8 max-w-2xl mx-auto w-full relative z-10 py-12">
          ...
          >
            ...
            {attachedFile && (
              <div className="flex items-center gap-2 p-2 rounded-xl bg-[#131415] border border-[#2d3131]/60 text-xs text-slate-300 mb-2 w-max animate-fade-in shrink-0 select-none">
                <span>📄</span>
            ...
            <div className="flex items-center justify-between mt-3 pt-2 border-t border-[#2d3131]/20">
              <div className="flex items-center gap-2 relative">
                ...
                >
                  <svg
                    className="w-3.5 h-3.5 text-[#20808D]"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth="2.5"
                ...
                {hasSupport && (
                  <button
                    ...
                    className={`flex items-center gap-1.5 text-xs font-semibold border px-3 py-1.5 rounded-lg transition duration-150 cursor-pointer ${
                      isListening
                        ? "bg-red-500/20 border-red-500 text-red-400 animate-pulse"
                        : "bg-[#131415]/60 hover:bg-[#202222] border-[#2d3131]/60 text-slate-300 hover:text-white"
                    }`}
                    title={
                      isListening
        ...
        <footer className="w-full text-center py-6 text-[11px] text-slate-600 border-t border-[#2d3131]/10 mt-auto relative z-10">
          &copy; {new Date().getFullYear()} Perplexity Clone. Built with React &
          Tailwind.
        </footer>
      </div>
